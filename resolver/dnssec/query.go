...
// queryBudget is the mutable per-validation upstream-query budget. It is shared through
// the context BY POINTER so every nested DS/DNSKEY sub-query draws from the same pool:
// with a value-typed budget, the decremented context was discarded whenever a callee did
// not thread it back up, so the documented maxUpstreamQueries cap never tripped.
type queryBudget struct{ remaining atomic.Int32 }

// withQueryBudget returns a context carrying a fresh shared budget of n upstream queries.
func withQueryBudget(ctx context.Context, n int) context.Context {
	b := &queryBudget{}
	b.remaining.Store(int32(n)) //nolint:gosec // n is a small configured cap
...
// consumeQueryBudget consumes one query from the shared budget and returns an error if
// the budget is exhausted. This provides DoS protection by limiting the number of
// upstream queries per validation.
...
// The returned context is the caller's context unchanged (the budget is shared by
// pointer, so no threading is needed); it is kept in the signature for call-site
// stability.
